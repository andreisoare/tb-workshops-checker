...
CheckCmd.prototype.runTaskTests = function(taskId, orderNumber, done) {
  ...
  var testPath = path.join(config.localFolder, 'task-' + orderNumber + '.js')
  nexpect.spawn('nodejs run-test ' + testPath + ' --colors', { cwd: config.localFolder, verbose: true })
        .run(function(err, stdout, exitCode) {
          if (exitCode === 0) {
            _this.cli.ok('Task ' + orderNumber + ' complete!\n\n')
            _this.markTaskComplete(taskId, done)
          } else {
            done(new CheckerError('Unfortunately this task is not complete yet!'))
          }
        })
}
